...
19. **THE ATLAS, THE PLAYER FILE, THE TABLE - done 2026-09-19** (see `docs/history/55-session-2026-09-19-atlas-rebuild-player-file-and-the-table.md`): `/atlas` rebuilt with hero mode, turf, a shot-density heat layer and a story strip; `/player/:id` gained WHERE HE SHOOTS (his shots over the league's density field), THE RACE (cumulative goals v xG across every match on record, gap filled green/red) and THE MARKET (ownership + per-GW transfer in/out sawtooth); new `/table` with THE QUADRANT (xG created v xG allowed per match, twenty crests) over a table carrying FINISHING/KEEPING. Payload builders may declare `needs_context = False` (`live/sse_server.py`) and are served from `light_context` - atlas, receipts, table, player and club profiles all do, and answer in milliseconds instead of behind a 60-130s context rebuild.
