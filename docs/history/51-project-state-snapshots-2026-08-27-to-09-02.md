# Superseded PROJECT_STATE snapshots (2026-08-27 to 2026-09-02)

Moved out of `docs/PROJECT_STATE.md` on 2026-09-10. That file is the
"read this first when resuming" document, and it had accumulated roughly
twenty stacked `## Where things stand` snapshots going back to 2026-08-27,
each superseded by the one above it. A resume document you have to read
backwards through twenty obsolete snapshots to find the current state is
not doing its job.

The two most recent snapshots stayed. Everything below is verbatim, in
original order.
