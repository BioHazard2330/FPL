...
def test_get_api_football_key_returns_none_when_unset(monkeypatch):
    monkeypatch.delenv("API_FOOTBALL_KEY", raising=False)
    assert config_mod.get_api_football_key() is None


def test_get_api_football_key_returns_value_when_set(monkeypatch):
    monkeypatch.setenv("API_FOOTBALL_KEY", "abc123")
    assert config_mod.get_api_football_key() == "abc123"
    monkeypatch.delenv("API_FOOTBALL_KEY", raising=False)
