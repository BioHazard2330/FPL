...
| Real team-level match odds (h2h + totals) via api-football.com, real global freshness throttle (`app_meta['api_football_odds_last_synced_at']`, `config/freshness.yaml`'s `api_football_odds` key) - replaced the-odds-api.com 2026-09-13 after that vendor's real free tier (500 credits/MONTH, not per day) was blown through in days by a real, disclosed missing-throttle bug at the old call site | `ingestion/api_football_odds_source.py` |
...
- No secrets committed. `.env` for tokens (`API_FOOTBALL_KEY`, currently the only one - replaces `ODDS_API_KEY` 2026-09-13). FPL account login is optional; the system works without it (no write-back to any account, ever).
...
- **the-odds-api.com removed entirely, 2026-09-13 (real root cause of a direct user report: "ran out of monthly credits and idk how")** — the earlier "HTTP 401 since 2026-09-07" entry this replaced was a real, correct symptom, but its own read (a stale/invalid key) was incomplete: real research found the-odds-api.com's free tier is 500 CREDITS PER MONTH, not the 500-requests-PER-DAY this project's own removed `odds_live_source.py` assumed in its own docstring - and that module's `sync_live_odds` call had ZERO internal throttle, firing on every single `run_scheduled` tick (adaptive down to ~15min near a deadline per this file's own Runtime/automation section). That combination could burn a whole month's budget in days - a genuine, disclosed bug, not evidence the free tier itself was too small. Real research into free alternatives before removing anything: SportsGameOdds' free tier explicitly excludes EPL (its own pricing page); odds-api.io's free tier has new signups "paused indefinitely" (its own pricing page); Betfair Exchange's real-time API needs a real GBP 299 one-off fee (violates free-resources-only). **api-football.com is the real, currently-signup-able, confirmed-free replacement** for team-level match odds (100 requests/day flat, EPL covered, `/odds` endpoint free-tier-included) - see `ingestion/api_football_odds_source.py`'s own module docstring for the full account, and Critical modules table above. Needs the user's own free signup + `API_FOOTBALL_KEY` in `.env` (never entered by Claude); the connector degrades to a clean no-op without it, same pattern every optional source here already uses. **Real, disclosed gap: no free anytime-goalscorer PLAYER PROP source was found** - `ingestion/player_odds_source.py` was removed rather than left pointed at a dead vendor; `_market_blended_share` (the blending code itself) is untouched and stays ready for whatever real source turns up, degrading cleanly to the unchanged model share until then. One real, not-yet-live-verified risk to watch: the new connector's exact JSON parsing (`bookmakers[].bets[].values[]`, bet names "Match Winner"/"Goals Over/Under") was built from api-football's own publicly documented schema since their docs site blocked automated fetches (Cloudflare) while building this - confirm it actually matches real payloads the first time a key is configured (`fpl doctor`/`fpl source-status` will show `api_football` DEGRADED if the real shape differs, never a silent wrong answer).
