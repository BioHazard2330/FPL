...
| Assists now respond to fixture difficulty via team_goals the same real way goals already did, and a real correlation gap ("independent assist draws violate the real team-goals ceiling") is closed - both the point estimate and the Monte Carlo joint-attribution use the same real, empirical assisted-goal-rate + assist-share (2026-09-13) | `models/player_regression.py::player_share_of_team_assists`/`real_assisted_goal_rate`, `models/expected_points.py::_match_components`'s assists term, `models/scenario_sampling.py` |
| Bonus points correlate with a player's own goals/assists within the same Monte Carlo trial, grounded in FPL's real official BPS table (2026-09-13) | `models/scenario_sampling.py::_bonus_correlation_multiplier` |
| Real anytime-goalscorer market odds (properly devigged via the actual "No" price, not the bare vigged "Yes" price) blended into a player's own per-fixture goal-share, mirroring the existing team-level odds blend (2026-09-13 - degrades to the unchanged model share whenever no real `player_odds_live` row exists for that player+fixture; the-odds-api.com-specific ingestion that fed this was removed 2026-09-13 with no free replacement found, see known-blockers entry) | `models/odds_devig.py::devig_two_outcome_prop`, `models/expected_points.py::_market_blended_share` |
| Real team-level match odds (h2h + totals) via the-odds-api.com, real global freshness throttle (`app_meta['odds_api_last_synced_at']`, `config/freshness.yaml`'s `odds_api` key, default 6h) - the real, root-cause fix (2026-09-13) for the missing-throttle bug that originally burned a month's credits in days; briefly replaced with api-football.com the same day, which turned out to be a real dead end (its free plan hard-restricts all data to seasons 2022-2024) - see known-blockers entry for the full twice-corrected account | `ingestion/odds_live_source.py` |
...
- No secrets committed. `.env` for tokens (`ODDS_API_KEY`, real and live; `API_FOOTBALL_KEY`, real but not wired into any live sync - see known-blockers entry). FPL account login is optional; the system works without it (no write-back to any account, ever).
...
- **Sync**: `sync`, `sync-history`, `sync-news`, `sync-live-odds`, `sync-eo`, `sync-predicted-lineups`, `sync-lineup-probability`, `sync-match`, `sync-crests`, `sync-elite-panel`, `solio-sync`, `backfill-odds`, `backfill-xg`, `backfill-cross-league`, `repair-understat-players`, `my-team`.
...
- **Player goals/xA rate recency-weighting was tested and rejected (2026-09-13, "make the optimizer smarter" pass)** — hypothesis: `player_regression.py::player_shrunk_rates`'s flat season-cumulative sums (no recency decay at all, unlike the team-strength half-life above) might be under-reacting to a player's recent hot/cold streak. Exploratory-only script (not committed) applied an exponential half-life decay (365/200/120/90/60/30 days) to the same per-player goals-rate walk-forward MAE check used for team strength, across 2023-24/2024-25/2025-26. Result: the flat, undecayed sum had the LOWEST weighted MAE of every candidate (0.1710), and MAE rose monotonically as the half-life shortened (0.1713 at 365 days down to 0.1731 at 30 days) - recency-weighting a player's own scoring rate measurably WORSENS real historical accuracy, the same direction of finding as the team-strength half-life test above. Nothing was changed - `player_shrunk_rates` keeps its flat-sum approach.

- **A short-rest-gap effect on a player's own next-match minutes is real but too small to build (2026-09-13)** — hypothesis: a player coming off a 60+-minute appearance on a short rest gap (<=4 days) plays fewer minutes next time than one on a normal rest gap (>=6 days), a real rotation-risk signal `expected_minutes.py` doesn't currently model. Exploratory-only script (not committed) checked this directly against `player_match_stats_history` across all 6 real seasons in the DB. A real gap exists in the hypothesised direction, but it is small enough relative to the rest of the model's own noise that it wasn't judged worth the added complexity of a new minutes-model term - not built. Revisit if a future in-season congestion period (multiple away legs / cup replays) makes the effect look larger in-season than the flat historical average suggests.
...
- **the-odds-api.com: removed, then restored, same day (2026-09-13) - real root cause was a missing throttle, not the vendor.** Direct user report: "ran out of monthly credits and idk how." Real cause: the-odds-api.com's free tier is 500 CREDITS PER MONTH, not the 500-requests-PER-DAY `odds_live_source.py`'s own docstring assumed - and `sync_live_odds` had ZERO internal throttle, firing on every `run_scheduled` tick (adaptive down to ~15min near a deadline). That alone burns a month's budget in days. First response: removed the vendor entirely, replaced with api-football.com (100 requests/day flat, EPL covered on paper). **That replacement was itself a real, live-verified dead end within the same day**: the user supplied a real free api-football.com key, and the very first live call returned `{"errors": {"plan": "Free plans do not have access to this season, try from 2022 to 2024."}}` - their free plan hard-restricts ALL data (not just odds) to seasons 2022-2024, useless for a live, current-season tool. Every other free alternative checked was also a real dead end: SportsGameOdds' free tier excludes EPL (its own pricing page); odds-api.io's free tier has new signups "paused indefinitely" (confirmed directly on their own live site, not just a secondary source); Betfair Exchange's real-time API needs a real GBP 299 one-off fee. **Resolution: restored the-odds-api.com, this time with the actual missing piece** - `odds_live_source.py::should_sync`, a real `app_meta`-backed cadence gate (`config/freshness.yaml`'s `odds_api` key, default 6h) - real budget math: one bulk call = 2 credits, at most 4 calls/day = 8 credits/day = ~240/month, comfortable margin under the real 500/month budget. The user's real `ODDS_API_KEY` is unchanged and still in `.env`; its credits should reset on its next monthly billing cycle. `API_FOOTBALL_KEY` is kept in `.env`, unused by any live sync, as a real disclosed possible future data point for 2022-2024 historical backtesting work only (`config.py::get_api_football_key`). **Real, disclosed gap, unchanged: no free anytime-goalscorer PLAYER PROP source was found** - `ingestion/player_odds_source.py` stays removed; `models/expected_points.py::_market_blended_share` (the blending code itself) is untouched and stays ready for whatever real source turns up, degrading cleanly to the unchanged model share until then. Re-reviving a player-prop sync later must budget its own real per-fixture cost against this SAME 500/month pool - do the math again before wiring it back in, don't just copy the old cadence.
